...
class FileMover:
    ...
    def _read_operations_log(self) -> List[Dict[str, Any]]:
        """Read operations from organizerr-api. Caller must already hold lock if concurrency matters."""
        try:
            operations = self.torrent_metadata.get_all_file_operation()
        except Exception as e:
            self.logger.warning(f"Failed to get operations: {e}")
            return []
        return operations

    def _load_operations_log(self):
        """Load operations log from organizerr-api into memory (best-effort snapshot)."""
        try:
            if not self.operations_enabled:
                self.operations_log = []
                return
            self.operations_log = self._read_operations_log()
